...
SymmetricKeyAlgorithmProvider.OpenAlgorithm(SymmetricAlgorithmNames.AesCbcPkcs7);
        ...
        private static byte[] key = new byte[] { 65, 69, 87, 65, 49, 50, 94, 64, 35, 77, 65, 87, 40, 0, 0, 0 };

        public static byte[] Key { get { return key; } }

        public static IBuffer KeyBufer
        {
            get
            {
                var dataWriter = new DataWriter();
    ...
    SymmetricKeyAlgorithmProvider.OpenAlgorithm(SymmetricAlgorithmNames.AesCbcPkcs7);
            ...
            CryptographicKey key =
                aesCbcPkcs7.CreateSymmetricKey(dataWriter.DetachBuffer());
            // Encrypt the data
            byte[] plainText = Encoding.UTF8.GetBytes("desktopad"); // Data to encrypt

            byte[] cipherText = CryptographicEngine.Encrypt(
                key, plainText.AsBuffer(), dataWriter.DetachBuffer()).ToArray();

            var str = Convert.ToBase64String(cipherText, 0, cipherText.Length);
            ...
            string newPlainText = new string(
                Encoding.UTF8.GetChars(CryptographicEngine.Decrypt(
                    key, cipherText.AsBuffer(), dataWriter.DetachBuffer()).ToArray()));
